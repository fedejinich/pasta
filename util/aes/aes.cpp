...
inline uint8_t getSBoxValue(int num) { return sbox[num]; }
inline uint8_t getSBoxInvert(int num) { return rsbox[num]; }
...
std::array<std::array<uint8_t, 4>, 4>& stateView(block& state) {
  return *(std::array<std::array<uint8_t, 4>, 4>*)&state;
}
...
inline void SubBytes(block& state_) {
  ...
  for (uint64_t i = 0; i < 16; ++i) state[i] = getSBoxValue(state[i]);
}
...
template <AESTypes type>
void AES<type>::ecbEncBlocks(const block* plaintexts, uint64_t blockLength,
...
template <AESTypes type>
void AES<type>::ecbEncTwoBlocks(const block* plaintexts,
                                block* ciphertext) const {
...
template <AESTypes type>
void AES<type>::ecbEncFourBlocks(const block* plaintexts,
                                 block* ciphertext) const {
  ciphertext[0] = plaintexts[0] ^ mRoundKey[0];
  ciphertext[1] = plaintexts[1] ^ mRoundKey[0];
...
template <AESTypes type>
void AES<type>::ecbEnc16Blocks(const block* plaintexts,
                               block* ciphertext) const {
  ciphertext[0] = plaintexts[0] ^ mRoundKey[0];
  ciphertext[1] = plaintexts[1] ^ mRoundKey[0];
  ciphertext[2] = plaintexts[2] ^ mRoundKey[0];
...
template <AESTypes type>
void AES<type>::ecbEncCounterMode(block baseIdx, uint64_t blockLength,
                                  block* ciphertext) const {
...
static void InvSubBytes(block& state_) {
  uint8_t* state = (uint8_t*)&state_;
  for (auto i = 0; i < 16; ++i) state[i] = getSBoxInvert(state[i]);
}
...
void AESDec<type>::ecbDecBlock(const block& ciphertext, block& plaintext) {
  ...
  // std::cout << "dec[10] " << plaintext << " ^ " << mRoundKey[10] <<
  // std::endl;
