...
class BFVTestSuite {
 ...
 public:
  ...
  bool hhe_test() {
      ...
      std::cout << "Encrypting message with PASTA..." << std::flush;
      typename T::Plain plain_cipher(key, plain_mod);
      time_start = std::chrono::high_resolution_clock::now();

      std::vector<uint64_t> ciph = plain_cipher.encrypt(plaintext);
      time_end = std::chrono::high_resolution_clock::now();
      time_diff = std::chrono::duration_cast<std::chrono::milliseconds>(
              time_end - time_start);
      std::cout << "...done" << std::endl;
      ...
      std::cout << "Encrypting symmetric key with BFV..." << std::flush;
      time_start = std::chrono::high_resolution_clock::now();
      cipher.encrypt_key(USE_BATCH);
      time_end = std::chrono::high_resolution_clock::now();
      time_diff = std::chrono::duration_cast<std::chrono::milliseconds>(
      ...
      std::cout << "Transciphering from PASTA to BFV..." << std::flush;
      time_start = std::chrono::high_resolution_clock::now();
      std::vector<seal::Ciphertext> transciphered_vec = cipher.HE_decrypt(ciph, USE_BATCH);
      time_end = std::chrono::high_resolution_clock::now();
      time_diff = std::chrono::duration_cast<std::chrono::milliseconds>(
              time_end - time_start);
      std::cout << "...done" << std::endl;
      std::cout << "Time: " << time_diff.count() << " milliseconds" << std::endl;
      std::cout << "noise:" << std::endl;
      cipher.print_noise(transciphered_vec);

      std::cout << "HE postprocessing..." << std::flush;
      time_start = std::chrono::high_resolution_clock::now();
      if (rem != 0) {
          std::vector<uint64_t> mask(rem, 1);
          cipher.mask(transciphered_vec.back(), mask);
      }
      seal::Ciphertext transciphered;
      cipher.flatten(transciphered_vec, transciphered);
      time_end = std::chrono::high_resolution_clock::now();
      time_diff = std::chrono::duration_cast<std::chrono::milliseconds>(
              time_end - time_start);
      std::cout << "...done" << std::endl;
      std::cout << "Time: " << time_diff.count() << " milliseconds" << std::endl;
      std::cout << "noise:" << std::endl;
      cipher.print_noise(transciphered);

      // todo(fedejinich) add an HE computation

      std::cout << "Final BFV decrypt..." << std::flush;
      time_start = std::chrono::high_resolution_clock::now();
      matrix::vector decrypted(N);
      cipher.packed_decrypt(transciphered, decrypted, N);
      time_end = std::chrono::high_resolution_clock::now();
      time_diff = std::chrono::duration_cast<std::chrono::milliseconds>(
              time_end - time_start);
      std::cout << "... done" << std::endl;
      ...
      decrypted.resize(4); // todo(fedejinich) try to avoid this

      if (plaintext != decrypted) {
          std::cerr << cipher.get_cipher_name() << " KATS failed!\n";
          utils::print_vector("plain:  ", plaintext, std::cerr);
          utils::print_vector("cipher: ", decrypted, std::cerr);
          return false;
      }
      return true;
  }
